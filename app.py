...
# Оновлений список рідин з коефіцієнтами Андраде (A та B) для розрахунку в'язкості від температури
liquid_options = {
    "Гліцерин": {"rho": 1260.0, "A": 1.12e-7, "B": 4900.0},
    "Касторова олія": {"rho": 961.0, "A": 2.4e-6, "B": 3780.0},
    "Машинна олія (SAE 40)": {"rho": 888.0, "A": 1.2e-5, "B": 3000.0},
    "Рідкий мед": {"rho": 1420.0, "A": 3.2e-9, "B": 6500.0},
    "Згущене молоко": {"rho": 1300.0, "A": 4.5e-6, "B": 4000.0},
    "Цукровий сироп (70%)": {"rho": 1350.0, "A": 8.0e-7, "B": 3850.0},
}

liquid_name = st.sidebar.selectbox("Виберіть рідину:", list(liquid_options.keys()))
rho_f = liquid_options[liquid_name]["rho"]

# Розрахунок динамічної в'язкості за формулою Андраде залежно від температури
A_const = liquid_options[liquid_name]["A"]
B_const = liquid_options[liquid_name]["B"]
eta = A_const * np.exp(B_const / T_K) 
...
# ДОДАЄМО СЮДИ: Ползунок температури (від 10°C до 80°C)
T_C = st.sidebar.slider(
    "Температура рідини T (°C):", min_value=10, max_value=80, value=20, step=5
)
T_K = T_C + 273.15  # Переводимо в Кельвіни для розрахунків
...
df_info = pd.DataFrame({
    ...
    "Значення": [
        f"{T_C}",
        f"{eta:.3f}", # покаже в'язкість з точністю до тисячних
        f"{rho_f}", 
        f"{rho_s}", 
        f"{r_mm}", 
        f"{L_distance:.2f}"
